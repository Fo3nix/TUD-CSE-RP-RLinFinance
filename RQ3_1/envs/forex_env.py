...
class ForexEnv(gym.Env):
    ...
    def __init__(self, df,
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.observation_space_shape,),
                                            dtype=np.float32)
    ...
    def _execute_trade(self, action, bid_price, ask_price):
        ...
        if action == 0:  # Go to CASH
            if self.position == 1:  # Was LONG
                ...
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Closing LONG. Sold {self.shares:.2f} at {bid_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                ...
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Closing SHORT. Covered {abs(self.shares):.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
            ...
            if self.position == -1:  # Close SHORT first
                ...
                self.cash -= (cost_to_cover + commission)
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Closing SHORT before LONG. Covered {abs(self.shares):.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                self.trade_history.append(
                    (self.current_step, "CLOSE_SHORT_FOR_LONG", ask_price, abs(self.shares), commission))
                self.shares = 0
                self.position = 0
            ...
            if self.position == 0 and self.cash > 0:
                if ask_price <= 0 or (ask_price * (1 + self.transaction_cost_pct)) <= 0:
                    if self.log_level > 0: print(
                        f"Step {self.current_step}: Cannot go LONG, invalid ask price or total cost. Ask: {ask_price}")
                    return
                ...
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Opened LONG. Bought {self.shares:.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash left: {self.cash:.2f}")
            ...
            if self.position == 1:  # Close LONG first
                ...
                self.cash += sell_value - commission
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Closing LONG before SHORT. Sold {self.shares:.2f} at {bid_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                self.trade_history.append(
                    (self.current_step, "CLOSE_LONG_FOR_SHORT", bid_price, self.shares, commission))
                self.shares = 0
                self.position = 0
            ...
            if self.position == 0 and self.cash > 0:
                if bid_price <= 0:
                    if self.log_level > 0: print(
                        f"Step {self.current_step}: Cannot go SHORT, invalid bid price: {bid_price}")
                    return
                ...
                if self.log_level > 0: print(
                    f"Step {self.current_step}: Opened SHORT. Shorted {abs(self.shares):.2f} at {bid_price:.5f}. Notional: {notional_value_to_short:.2f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                self.trade_history.append((self.current_step, "OPEN_SHORT", bid_price, abs(self.shares), commission))
            elif self.cash <= 0 and self.log_level > 0:
                print(
                    f"Step {self.current_step}: Cannot go SHORT, insufficient cash for notional value: {self.cash:.2f}")

    def render(self, mode='human'):
        if mode == 'human':
            print(
                f"Step: {self.current_step}, Equity: {self.equity:.2f}, Shares: {self.shares:.2f}, Cash: {self.cash:.2f}, Position: {self.position}")
...
if __name__ == '__main__':
    try:
        ...
        dates = pd.to_datetime(
            [pd.Timestamp('2023-01-01 00:00:00') + pd.Timedelta(minutes=15 * i) for i in range(data_size)])
        ...
        dummy_data = {
            ...
            'volume': np.random.randint(100, 1000, data_size) * 100.0
            # Ensure volume is float for stockstats if needed
        }
    ...
    if episode_action_history_from_info is not None:
        ...
        print(
            "[Main] Using action history via direct access from underlying_eval_env (may be from reset state or incomplete episode if loop ended by range).")
